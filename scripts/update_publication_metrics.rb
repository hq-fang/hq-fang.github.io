...
def google_scholar_article_for(publication, google_scholar_index)
  ...
  if google_scholar_id && !google_scholar_id.empty?
    ...
    warn "Could not find Google Scholar article #{google_scholar_id} for #{publication['title']}; trying title match."
  end
...
def fetch_google_scholar_citation_count(publication, google_scholar_index)
  article = google_scholar_article_for(publication, google_scholar_index)
  article && (article.dig("cited_by", "value") || 0)
end

def google_scholar_citation_id_for(publication, google_scholar_index)
  article = google_scholar_article_for(publication, google_scholar_index)
  article && article["citation_id"]
end
...
def replace_google_scholar_id(content, google_scholar_id)
  return [content, false] unless google_scholar_id && !google_scholar_id.empty?

  pattern = /^(\s{2}google_scholar_id:\s*).*$/
  if content.match?(pattern)
    updated_content = content.sub(pattern, "\\1'#{google_scholar_id}'")
    return [updated_content, updated_content != content]
  end

  citation_index = content.lines.index { |line| line =~ /^citation:\s*$/ }
  return [content, false] unless citation_index

  lines = content.lines
  provider_index = nil
  ((citation_index + 1)...lines.length).each do |index|
    break if lines[index] =~ /^\S/

    if lines[index] =~ /^\s{2}provider:\s*/
      provider_index = index
      break
    end
  end
  return [content, false] unless provider_index

  lines.insert(provider_index + 1, "  google_scholar_id: '#{google_scholar_id}'\n")
  [lines.join, true]
end
...
publication_entries.each do |path, publication|
  ...
  if GOOGLE_SCHOLAR_PROVIDERS.include?(publication.dig("citation", "provider"))
    google_scholar_article = google_scholar_article_for(publication, google_scholar_index)
    citation_count = google_scholar_article && (google_scholar_article.dig("cited_by", "value") || 0)
    citation_count = publication.dig("metrics", "citation_count") if citation_count.nil?

    content, field_updated = replace_google_scholar_id(content, google_scholar_article && google_scholar_article["citation_id"])
